...
default runtime / package manager**. Content is sourced from the generated
[`../data/resumes.json`](../data/resumes.json) contract. Angular-ready — see
...
│   ├── lib/resume.ts    Typed selector for ../data/resumes.json
...
Imported at build time from `../data/resumes.json` via a TypeScript path alias. The site selects
the public `carlos-ferreyra` entry:
...
const resume = resumesData.resumes.find(
  (candidate) => candidate.slug === 'carlos-ferreyra',
);
```

`resumes.json` is generated by `uv run scripts/resume_build.py`. Edit `data/baseline.json`, a stack,
or a combo instead of editing the generated file.
